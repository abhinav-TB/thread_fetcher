    ...
    def add_tweet_card(self, tweet_text, tweet_media_type=None, tweet_media_urls=None) -> None:
        ...
        if tweet_media_type == "photo":
            for image_url in tweet_media_urls:
                self.body += f"""
                <img class="tweet_image" src="{image_url}" alt="There was some problem loading this image, turn off anti-tracking tools and try again.">
                """
        elif tweet_media_type == "animated_gif":
            self.body += f"""
            <video class="tweet_gif" controls autoplay loop src="{tweet_media_urls[0]}">
                There was some problem loading this GIF, turn off anti-tracking tools and try again.
            </video>
            """
        elif tweet_media_type == "video":
            self.body += f"""
            <video class="tweet_video" controls src="{tweet_media_urls[0]}">
                There was some problem loading this video, turn off anti-tracking tools and try again.
            </video>
            """
        self.body += "</div>"
